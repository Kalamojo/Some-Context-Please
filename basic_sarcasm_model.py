...
# saving
with open('Basic_tokenizer.pickle', 'wb') as handle:
...
model = Sequential()
model.add(Embedding(word_size, 12, input_shape=(2, padd_max)))
model.add(Flatten())
...
Y_test2 = labels # Convert one-hot to index
predict_x=model.predict(docs_x2)
classes_x=np.round(predict_x)
print(classification_report(Y_test2, classes_x))
